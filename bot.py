...
async def approve_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    for idx, row in enumerate(rows[1:], start=2):
        if row_value(row, 0) == pending_id:
            ...
            await update.message.reply_text(f"✅ Bazar approved and messages sent: {pending_id}")
            return
...
async def scan_bazar(bot, data: Dict[str, Any]) -> bool:
    ...
    for idx, row in enumerate(rows[3:], start=4):
        ...
        await asyncio.to_thread(
            ...
            idx,
            7,
            "SENT ADMIN ONLY" if repair_mode else f"SENT: {success}",
        )
...
async def auto_scan_loop(bot):
    ...
    while True:
        try:
            ...
            # Low wallet auto message removed by request.
...
async def normal_message_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    member = normalize_name(
        get_member_name_by_user_id(data, uid)
    ...
    if low in ["ok", "okay", "ওকে", "ঠিক আছে", "হ্যাঁ", "ha", "yes"]:
        ...
        if pending and pending.get("type") == "BAZAR":
            ...
            await send_admin(
                ...
                f"👤 Buyer: {member}\n"
                f"🧾 Items: {p['items']}\n"
                f"💰 Total: {format_lkr(p['total'])} LKR\n\n"
                f"Approve: /approve {p['id']}\n"
                f"Reject: /reject {p['id']}",
                data,
            )

            reply = "✅ বাজারটা admin approval-এ পাঠানো হয়েছে। Admin approve দিলেই sheet update + সবার message যাবে।"
...
def main():
    ...
    print("Market Hisab Bot - Clean Market Only running...")
